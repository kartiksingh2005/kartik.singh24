...
function App() {
  ...
  const storeUserDataOnBlockchain = async () => {
    try {
      ...
      // 1️⃣ Generate hash from user data
      const hash = generateUserHash();

      const params = await algodClient.getTransactionParams().do();

      // 2️⃣ Create SELF transaction with NOTE field
      const txn = algosdk.makePaymentTxnWithSuggestedParamsFromObject({
        sender: account,
        receiver: account,   // send to self (cheap + safe)
        amount: 0,
      ...
      // 4️⃣ Send to Algorand blockchain
      await algodClient.sendRawTransaction(signedTxn).do();
  ...
  const generateUserHash = () => {
    ...
    // Convert profile into SHA256 hash
